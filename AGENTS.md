...
AccessWeb v3 — a Russian-language logistics management web app that replaces a legacy MS Access desktop database (the name reflects that migration; there is no live `.mdb`/`.accdb` file anywhere in this repo). It manages cargo shipments (ACID), contracts (including a stage/substage tracker with a Chart.js Gantt view), counterparties, financial transactions (AIS), and includes a task/CRM module with a kanban board, notifications, an audit log, a cross-entity global search, and an ad-hoc SQL report builder.
...
- Backend: Node.js + Express (`backend/src/server.js`, ~1760 lines, single file), using SQLite (`sqlite`/`sqlite3` packages) for storage, JWT for auth, `bcryptjs` for password hashing, `multer` for file uploads.
