...
- **Migrations**: `runMigrations()` applies idempotent schema changes on server startup; `backend/data/schema.sql` documents the base schema. Prefer adding a migration over hand-editing the live `.db` file. SQLite can't `ALTER` a `CHECK` constraint — widening one (e.g. adding an allowed `entity_type` value) means rebuilding the table: `CREATE TABLE x_new (...)` with the new constraint, `INSERT INTO x_new SELECT * FROM x`, `DROP TABLE x`, `RENAME TO x`, guarded by checking `sqlite_master.sql` for the new value first so it stays idempotent. See the `notifications` and `tasks` migrations in `runMigrations()` for the pattern.
