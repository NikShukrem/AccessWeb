...
- **Contract stage tracking**: `contract_stages` rows (stage_number, stage_name, substage_name, planned_date, actual_date, status, responsible) are managed through a dedicated "Стадии договоров" sub-tab (plain CRUD via the generic `/:table` routes) and visualized read-only as a Chart.js floating-bar Gantt in the "Этапы (Гант)" sub-tab. A template loader (`CONTRACT_STAGE_TEMPLATE` in `index.html`) can bulk-create the standard 4-stage/24-substage approval process from a chosen start date, computing each `planned_date` by chaining the *maximum* of its working-day duration range.
